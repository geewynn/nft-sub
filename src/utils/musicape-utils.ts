...
import { Musicape } from "../../generated/Musicape/Musicape";
...
export function getOrCreateCollection(address: Address): Collection {
  ...
  if (!collection) {
    ...
    let contract = Musicape.bind(address);
...
export function getOrCreateCollectible(collectionAddress: Bytes, collectionId: string, tokenId: BigInt, creatorId: string, createdTimestamp: BigInt) : Collectible {
    ...
    if(!collectible){
        ...
        // collectible.descriptorUri = Musicape.bind(Address.fromBytes(collectionAddress)).tokenURI(tokenId);
        collectible.save();
    }
    return collectible;
}